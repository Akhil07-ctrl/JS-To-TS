...
const Appointments: React.FC = () => {
  const [patientsData, setPatientsData] = useState<Appointment[]>([])
  const [patientsHeaders, setPatientsHeaders] = useState<string[]>([])
      ...
      <table className='w-full mt-4 mx-auto'>
        ...
        <tbody>
          {patientsData?.map((row, index) => (
            <tr key={index} className='border-b-2 border-slate-200 hover:shadow-gray-400/40 cursor-pointer' >
